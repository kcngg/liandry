  ...
  return {
    ...
    transform(code, id) {
      if (/\.(?:js|jsx|ts|tsx|vue|svelte|html)$/.test(id)) {
        ...
        const utilityRegex =
          /["'`]([^"'`]*(?:bg-|text-|p-|m-|w-|h-|flex|grid|rounded|border)[^"'`]*)["'`]/g
        ...
        this.debug(
          `UnoCSS: Processing ${id}, found ${extractedTokens.size} total tokens so far`,
        )
      }
    ...
    async generateBundle(outputOptions, bundle) {
      ...
      try {
        ...
        if (result.css) {
          ...
          this.info(
            `UnoCSS: Generated ${cssFileName} with ${result.css.length} bytes from ${extractedTokens.size} tokens.`,
          )
          ...
          if (options.injectCSS !== false) {
            ...
            if (entryChunk) {
              ...
              this.info(
                `UnoCSS: Injected CSS import into ${entryChunk.fileName}`,
              )
            }
          }
        }
